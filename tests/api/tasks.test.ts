...
describe("GET /api/tasks", () => {
  ...
  it("renvoie la liste des tâches → 200", async () => {
    await insertTask({ title: "Task 1", priority: "high" });
    await insertTask({ title: "Task 2", priority: "low" });
  ...
  it("filtre par priority=high → 200", async () => {
    await insertTask({ title: "High task", priority: "high" });
    await insertTask({ title: "Low task", priority: "low" });
  ...
  it("filtre par status=done → 200", async () => {
    await insertTask({ title: "Done task", status: "done" });
    await insertTask({ title: "Todo task", status: "todo" });
  ...
  it("filtre par category=Frontend → 200", async () => {
    await insertTask({ title: "FE", category: "Frontend" });
    await insertTask({ title: "BE", category: "Backend" });
  ...
  it("filtre par recherche texte q=login → 200", async () => {
    await insertTask({ title: "Bug login", description: "Fix needed" });
    await insertTask({ title: "Doc API", description: "Write docs" });
  ...
  it("filtre combinés (AND) → 200", async () => {
    await insertTask({ title: "Login FE", priority: "high", category: "Frontend" });
    await insertTask({ title: "Login BE", priority: "low", category: "Backend" });
    await insertTask({ title: "Doc FE", priority: "high", category: "Frontend" });
